namespace CityOfWisdomBot.Services
{
    using System;
    using System.IO;
    using System.Linq;
    ...
    public class MicrosoftCognitiveCaptionService : ICaptionService
    {
        ...
        private static string graffitiTag = AppConstants.GRAFFITI_TAG;
        private static string potholeTag = AppConstants.POTHOLE_TAG;
        /// <summary>
        ...
        private static string ProcessAnalysisResult(AnalysisResult result)
        {
            string serviceRequestType = string.Empty;

            if(result.Description.Tags.Any(x => x == graffitiTag))
            {
                serviceRequestType = AppConstants.GRAFFITI;
            }
            else if (result.Description.Tags.Any(x => x == potholeTag))
            {
                serviceRequestType = AppConstants.POTHOLE;
            }
